...
class GLA_module(nn.Module):
    def __init__(self,
        ...
        self.head_size = int(heads / self.num_heads)
    ...
    def transpose(self, x):
        new_shape = x.size()[:-1] + (self.heads, self.head_size)
        x = x.view(*new_shape)
    ...
    def forward(self, local_x, global_x):
        ...
        similarity = similarity / math.sqrt(self.attention_head_size)
...
class ds_FCRN(nn.Module):
    def __init__(self, inplace,
        ...
        """
            ...
            network: feature extraction network, default is FCRN
    ...
    def forward(self, input):
        ...
        global_output = F.softmax((torch.flatten(self.conv(self.drop((self.avg(global_x)))), 1)), dim=1)
            ...
            for x in range(0, W - self.step, self.step):
                for z in range(0, D - self.step, self.step):
                    ...
                    for n in range(self.nblock):
                        ...
                        tmp = self.attnlist[n](local_xt, global_xt)
                        tmp = tmp.permute(0, 2, 1)
                        tmp = tmp.view(B1, C1, H1, W1, D1)
                        tmp = torch.cat([tmp, local_x], 1)

                        tmp = self.fftlist[n](tmp)
                        local_x = local_x + tmp

                    fusion_outout = F.softmax((torch.flatten(self.conv(self.drop((self.avg(local_x)))), 1)), dim=1)
